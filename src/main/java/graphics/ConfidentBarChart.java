...
public class ConfidentBarChart extends Pane {
    ...
    String[] colors = {"#0288AB", "#ff9912", "#a23a77", "#84b203", "#17806d", "#fcb4d5", "#e3256b", "#bab86c", "#fce883",
    ...
    public void draw() {
        ...
        if(getHeight()>0) {
            ...
            Object[] autoRange = autoRange(min, max, this.getHeight() - maxLabelHeight - titleHeight, fontSize);
            min = (double) autoRange[0];
...
//        });
    ...
    public double makeYAxis(Object[] autoRange, double height) {

        //Object[]{minRounded, maxRounded, tickUnitRounded, newScale, formatter}

        double min = (double) autoRange[0];
        double max = (double) autoRange[1];


        double tickIndent = (double) autoRange[2];
        int nbTicks = (int) ((max-min)/tickIndent);
        double widestText = 0;
        DecimalFormat formatter = new DecimalFormat((String) autoRange[4]);

        for (double i = min; i <= max; i+=tickIndent) {
            Text text = new Text(formatter.format(i));
        ...
        for (double i = min; i <= max; i+=tickIndent) {
            ...
            text.setY(height - height / nbTicks * tickIndex + text.getLayoutBounds().getHeight() / 2);
        ...
        for (double i = min; i <= max; i+=tickIndent) {
            Line l = new Line();
            l.setStartY(height - height / nbTicks * tickIndex);
            l.setEndY(height - height / nbTicks * tickIndex);
