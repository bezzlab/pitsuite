...
scale.sets = "identity"; text.scale =2; set_size.angles = 0 ; set_size.show = FALSE; set_size.number_size=3
...
claude= read.csv(args[1])
#claude = read.csv("/home/esteban/Documents/PITGuiRef/PITGUIguidedassembly/Rscripts/variantsTable.csv")
claude=claude[!duplicated(claude[,1]), ]
claude=claude[, 2:ncol(claude)]

library(RColorBrewer)

qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
colours = col_vector[1:ncol(claude)]


data = claude; sets = colnames(claude); 
...
#   if(length(j) > 0) Matrix_layout$color[j] <- colours[i]
# }
...
Sizes <- UpSetR:::Make_size_plot(Set_sizes, sets.bar.color, mb.ratio, sets.x.label, scale.sets, text.scale,
                                 set_size.angles,set_size.show, set_size.number_size = 2, set_size.scale_max = max(Set_sizes))
...
jpeg(filename=args[2], width = as.numeric(args[3]), height = as.numeric(args[4]))
...
#png(filename="/media/data/Dropbox/PIT/output21")
#plot(fit)
dev.off()
# dev.print(jpeg, "/media/esteban/data/Dropbox/PIT/output21/test.jpeg", width=500)
